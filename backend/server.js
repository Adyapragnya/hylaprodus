...
app.post('/api/add-combined-data', async (req, res) => {
    try {
        ...
        const { IMO,'0': { AIS } = {}, SpireTransportType, FLAG, GrossTonnage, deadWeight } = req.body;
        ...
        const newCombinedData = new TrackedVessel({ IMO,AIS, SpireTransportType, FLAG, GrossTonnage, deadWeight,trackingFlag: true,lastFetchTime: currentTime,GeofenceStatus: null, geofenceFlag: null,GeofenceInsideTime: null });
...
app.get('/api/get-vessel-tracked-by-user', async (req, res) => {
  ...
  app.post("/api/add-vessels-bulk", async (req, res) => {
    const { imoNumbers, id } = req.body;
    const userkey = "WS-096EE673-456A8B"; // Replace with your actual API key
    ...
    try {
      ...
      const emailvs = "kdalvi@hylapps.com, abhishek.nair@hylapps.com,tech.adyapragnya@gmail.com,udhaykirank@adyapragnya.com,support@hylapps.com"; // Add your recipient email here
...
app.get('/api/sat-intervals', async (req, res) => {
  ...
  async function checkAndUpdateVesselData() {
      try {
          // Fetch the satellite pull configuration and geofences
          const aisSatPullConfig = await AisSatPull.findOne();
        //   console.log(aisSatPullConfig);
          if (!aisSatPullConfig) {
              console.error('Sat pull intervals not found.');
              return;
          }
  
          const vessels = await TrackedVessel.find(); // Get all vessels
          const TerrestrialGeofences = await TerrestrialGeofence.find(); // Get all geofences
        ...
        setInterval(() => {
            ...
            TerrestrialGeofences.forEach((geofence) => {
                ...
                if (isInside) {
                    ...
                    if (geofence.geofenceType === 'terrestrial') {
                        interval = aisSatPullConfig.sat0;
                        console.log(interval);
                        sat = 0;
                    } else if (geofence.geofenceType === 'boundary') {
                        interval = aisSatPullConfig.sat1a;
                        console.log(interval);
                        sat = 1;
                    }
          ...
          setTimeout(checkAndUpdateVesselData, 1000 * 60); // Runs the function every minute
      }
  }
